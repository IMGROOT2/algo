<template>
      <main class="flex items-center justify-center flex-col w-full mx-auto min-h-screen" v-show="!mockSetUp && !loading">
      <div class="w-full mx-auto text-center transition-all">
        <h1
          class="font-extrabold text-transparent text-4xl lg:text-5xl bg-clip-text mt-10 bg-gradient-to-tr from-[#30a7d6] to-[#27c857]">Mock Contest
        </h1>
            ...
            <div class="flex items-center justify-center w-screen-xl mx-auto gap-3">
              ...
              <div v-show="!msuProblemsLoading">
                <div id="diff-dropdown">
                  ...
                  <button class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-xs p-3 top-[2px] relative ml-1 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" @click="refresh">
    ...
    >
      <div class="relative w-full max-w-2xl max-h-full mx-auto">
        ...
        <div class="relative bg-white rounded-lg shadow dark:bg-zinc-800">
          ...
          <div class="flex items-start justify-between p-4 border-b rounded-t dark:border-gray-600">
            <h3 class="text-xl text-gray-900 dark:text-white"><span class="mr-2"><i class="fa-solid fa-clock"></i></span>Time's up!</h3>
          </div>
          <!-- Modal body -->
      ...
      <nav class="bg-white border-gray-200 dark:bg-zinc-900 w-full">
        <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
          <div class="flex items-center">
            ...
            <button class="text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-xs px-3 py-1.5 text-center inline-flex items-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" @click="endNow">End Now</button>
          </div>
          <button @click="mobilenav = !mobilenav" type="button" class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg md:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 border-gray-400 border-2 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-default" aria-expanded="false">
      ...
      <div class="mx-auto justify-center flex mt-10"><Loader size="16" v-show="problemToggleloading" /></div>
  ...
  async function generateMockProblems(db, user) {
    ...
    for(let i = 1; i <= 3; i++) {
      await retrieveUserDoc(db, user).then((adoc) => {
        ...
        let possibleProblem = problems[localStorage.getItem('mock-difficulty').toLowerCase()][Math.floor(Math.random() * problems[localStorage.getItem('mock-difficulty').toLowerCase()].length)]
        ...
        while (problemsSeen.includes(id) || array.includes(id) || number != i) {
          ...
          possibleProblem = problems[localStorage.getItem('mock-difficulty').toLowerCase()][Math.floor(Math.random() * problems[localStorage.getItem('mock-difficulty').toLowerCase()].length)]
